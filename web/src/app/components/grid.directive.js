...
import blockyScopeElement from './scope-element.directive';
import blockyDetailsSidenav from './details-sidenav.directive';
...
export default angular.module('blocky.directives.grid', [blockyScopeElement, blockyDetailsSidenav])
...
function GridController($scope, $state, $mdDialog, $document, $q, $timeout, $translate, $mdMedia, $templateCache) {
    ...
    function initGridConfiguration() {
        ...
        vm.hideAddItemButton = vm.config.hideAddItemButton || function () {
                return false;
            };
    ...
    function openItem($event, item) {
        ...
        if (vm.detailsConfig.currentItem != null && vm.detailsConfig.currentItem._id === item._id) {
    ...
    function isCurrentItem(item) {
        if (item != null && vm.detailsConfig.currentItem != null &&
            vm.detailsConfig.currentItem._id === item._id) {
    ...
    function deleteItem($event, item) {
        ...
        $mdDialog.show(confirm).then(function () {
                vm.deleteItemFunc(item._id).then(function success() {
                    refreshList();
                });
            },
            function () {
    ...
    function toggleItemSelection($event, item) {
        ...
        if (item.selected) {
            vm.items.selections[item._id] = true;
            vm.items.selectedCount++;
        } else {
            delete vm.items.selections[item._id];
            vm.items.selectedCount--;
        }
    }
